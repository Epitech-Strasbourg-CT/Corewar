...
** Last update Tue Mar 28 17:53:49 2017 
*/
...
static int	particular(unsigned char *champ, int index, t_ins *ret)
{
  if (ret->cmd == 1)
    {
      ret->type[0] = 2;
      ret->tot_byte = 5;
      ret->val[0] = extract_val(champ + index + 1, 4);
      return (1);
    }
  if (ret->cmd == 9 || ret->cmd == 12 || ret->cmd == 15)
    {
      ret->type[0] = 2;
      ret->tot_byte = 3;
      ret->val[0] = extract_val(champ + index + 1, 2);
      return (1);
    }
  return (0);
}

int	get_arg(unsigned char *champ, int index, t_ins *ret)
{
  int	i;
  int	decal;

  ret->tot_byte = 0;
  if (particular(champ, index, ret))
    return (0);
  i = -1;
  decal = 0;
  while (++i < ret->nb_arg)
    {
      ret->val[i] = extract_val(champ + index + 2 + decal,
